function [ARES_background] = compute_ARES_background_matrixwise_RAM (img, images_stack_info, selected_ROI, tag_array, opts_ARES_film)
...
if (nargin < 5) || isempty(opts_ARES_film)
    ...
    opts_ARES_film.window_g_std = 20;
...
for i_image = 1:window_advance_step:(number_of_frames - window_length - 1)
    ...
    if (tag_array(1, i_image) == 1 && tag_array(1, i_image + window_length) == 1)
            ...
            while i_pixel <= image_Height && pixel_counter <= matrix_dimension
                while j_pixel <= image_Width && pixel_counter <= matrix_dimension
                    if (selected_ROI(i_pixel, j_pixel) == 1) % Use only ROI pixels.
                        current_pixel_time_series(:) = double(images_stack(i_pixel, j_pixel, 1:end));
        ...
        ARES_img_tmp = nonzeros(ARES_img); % Exclude zeros from the computation.
