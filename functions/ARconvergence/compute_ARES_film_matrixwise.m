function [ARES_film, ARES_film_info] = compute_ARES_film_matrixwise (images_stack, images_stack_info, selected_ROI, tag_array, opts_ARES_film)
...
if (nargin < 5) || isempty(opts_ARES_film)
    ...
    opts_ARES_film.window_type = 'rectangular';
    opts_ARES_film.window_g_std = 6;
    opts_ARES_film.window_advance_step = 2;
    opts_ARES_film.window_length = 25;
    opts_ARES_film.process_order = 3;
    opts_ARES_film.matrix_dimension = 50;
    opts_ARES_film.absolute_value = 0;
    opts_ARES_film.FLAG_demean = 0;
...
for i_image = 1:window_advance_step:(number_of_frames - window_length)
    ...
    if (tag_array(1, i_image) == 1 && tag_array(1, i_image + window_length) == 1)
                ...
                while j_pixel <= image_Width && pixel_counter <= matrix_dimension
                    if (selected_ROI(i_pixel, j_pixel) == 1) % Use only ROI pixels.
                        if opts_neighbourhood_ts.neighbourhood_order ~= 0 % Use neighbourhood time series instead of single pixel, if required.
                            ...
                            current_pixel_TS(:) = double(images_stack(i_pixel, j_pixel, i_image:(i_image + window_length - 1)));
        ...
        if counter_frame == 2
            ...
            fprintf('\nEstimated time to complete: %dh:%dm:%ds.\n', time_est_hour, time_est_m, time_est_s);        end
...
ARES_film_info.date_analised = date_str;
...
ARES_film_info.FLAG_RemovedOrInterpolatedFrames = images_stack_info.FLAG_RemoveOrInterpolate;
