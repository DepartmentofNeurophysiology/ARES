function save_output_avi_v3 (ARES_film, ARES_film_info, ElectroData_Whole_TS, options, user_selected_projection)
...
if options.overlay_EPhys == 1
    ...
    axis_3.Position = [0.605 0.05 0.3 0.130];% [left bottom width height];
    set(gca, 'Color', 'none')
...
%         truesize;
    ...
    if options.overlay_EPhys == 1
        ...
        % Plot
        plot(axis_3, E_Phys_current_time, E_Phys_current_segment, 'r')
        axis([-inf, inf, E_Phys_min, E_Phys_max])
        X_ticks_tmp = get(axis_3, 'XTick');
%         X_ticks = [X_ticks_tmp(1), X_ticks_tmp(end)];
